    ...
    if uploaded:
        ...
        if score >= 80:
            ...
            if both_neutral and p1 < 60:
                st.warning(f"グレー・ベージュ系でまとまったコーデですが、色が分散しています（最大色{p1:.0f}%、目標70%）。上下をより近い色で統一するとベース割合が上がります。")
            elif not p1_is_neutral and p1 < 60:
                st.warning(f"ベースカラー（{base_name}）が{p1:.0f}%と少なめです（目標70%）。方向性は2つあります。①{base_name}系のアイテムをもう1枚足してベースを強化する、②{base_name}を差し色（5%）に絞って上下を無彩色でまとめる。どちらかに振り切るとスコアが上がります。")
            elif p1 > 85:
